...
static void ds18b20AddBitToRom(ds18b20 *this,uint8_t bit){
	if(this->romAddedBit>=(8*8)){
		return;
	}
	uint8_t array=this->romAddedBit%8;
	uint8_t bitToSave=this->romAddedBit/8;
	this->romNO[array]=(bit&0x01)<<bitToSave;
	this->romAddedBit++;

}
// global search state
volatile unsigned char ROM_NO[8];
volatile int LastDiscrepancy;
volatile int LastFamilyDiscrepancy;
volatile int LastDeviceFlag;
...
//
void OWTargetSetup(ds18b20* this,unsigned char family_code)
{
   int i;
...
// to OWNext().
//
void OWFamilySkipSetup(ds18b20* this)
{
